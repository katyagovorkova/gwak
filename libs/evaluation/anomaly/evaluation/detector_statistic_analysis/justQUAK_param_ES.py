...
from anomaly.evaluation import QUAK_stats
import time
...
def score_parameters(param_vec, files):
    TS_QUAK_TRAIN, timeslide_QUAK, signal_QUAK_flat, signal_QUAK = files
    #print("SIGNAL QUAK FLAT", signal_QUAK_flat)
    #np.save("/home/ryan.raikman/s22/temp2/TS_QUAK_data.npy", TS_QUAK_TRAIN)
    discriminator = QUAK_stats(TS_QUAK_TRAIN, weights=param_vec)

    scores = discriminator(signal_QUAK_flat)
    scores = np.reshape(scores, (len(scores), 1)) #should go back to what this originally was
    ...
    score_timeslides = np.reshape(score_timeslides, (score_timeslides.size, 1))#, timeslide_pearson.shape)
...
def main(timeslide_path, signal_path):
    ...
    files = [TS_QUAK_TRAIN, timeslide_QUAK, signal_QUAK_flat, signal_QUAK]
