...
def build_model_from_save(savedir):
    model_params = np.load(f"{savedir}/TRAINED_MODELS/PEARSON_QUAK_STATS/v2_model_params.npy")
    weights = np.load(f"{savedir}/TRAINED_MODELS/PEARSON_QUAK_STATS/v2_model_params_QUAK_weights.npy")
    print("model_params", model_params)


    pearson_bc_lmbda, mu_pearson, sigma_pearson, km_shift, QUAK_bc_lmbda, mu_QUAK, sigma_QUAK = model_params

    return make_full_discriminator(pearson_bc_lmbda, mu_pearson, sigma_pearson, 
                                    km_shift, QUAK_bc_lmbda, mu_QUAK, sigma_QUAK, weights)


def make_full_discriminator(pearson_bc_lmbda, mu_pearson, 
                                          sigma_pearson, km_shift, 
                                          QUAK_bc_lmbda, mu_QUAK, sigma_QUAK, weights):
    '''
...
def main(train_QUAK, train_pearson, savedir=None, weights = np.array([1, -0.5, -1.5, 1])):
    '''
    ...
    train_QUAK_mu_sig = train_QUAK[len(train_QUAK)//2:]
    ...
    km_QUAK = katya_metric(train_QUAK_lmbda, weights)
    ...
    km_QUAK_bc_mu_sig = boxcox(katya_metric(train_QUAK_mu_sig, weights)+km_shift, QUAK_bc_lmbda)
    mu_QUAK, sigma_QUAK = norm.fit(km_QUAK_bc_mu_sig)
