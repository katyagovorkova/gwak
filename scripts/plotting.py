...
def snr_vs_far_plotting(data, snrs, metric_coefs, far_hist, tag, savedir):
    ...
    plt.legend()
    plt.xlim(VARYING_SNR_LOW - SNR_VS_FAR_BAR / 2,
    ...
    if do_snr_vs_far:
        ...
        for tag in tags:
            ...
            if RETURN_INDIV_LOSSES:
                ...
                #final_values = model(final_values).detach()
                snr_vs_far_plotting(
                    data[1000:], snrs[1000:], model, far_hist, tag, args.plot_savedir)
            else:
                snr_vs_far_plotting(
                    data[1000:], snrs[1000:], metric_coefs, far_hist, tag, args.plot_savedir)
            #
    if do_fake_roc:
        far_hist = np.load(f"{args.data_predicted_path}/far_bins.npy")
    ...
    if do_3_panel_plot:
        ...
        for tag in tags:
            ...
            if RETURN_INDIV_LOSSES:
                ...
                three_panel_plotting(strains[:, 1000:], data[
                                     1000:], snrs, model, far_hist, tag, args.plot_savedir)
            else:
                #snr_vs_far_plotting(data[1000:], snrs[1000:], metric_coefs, far_hist, tag, args.plot_savedir)
                three_panel_plotting(
                    strains, data, snrs, metric_coefs, far_hist, tag, args.plot_savedir)
