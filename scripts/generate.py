...
def get_bkg_segs(
    ...
    quiet_times_H1 = get_quiet_segments(
        loaded_data["H1"], N, args.segment_length)
    quiet_times_L1 = get_quiet_segments(
        loaded_data["L1"], N, args.segment_length)
...
def inject_waveforms(
    ...
    for i, ifo in enumerate(IFOS):
        ...
        # make full seg, basically a segment with all the bkg_segs appended
        # together
        print('BKG SEG SHAPE', bkg_segs.shape,
              'hstack', np.hstack(bkg_segs).shape)
        full_seg = TimeSeries(np.hstack(bkg_segs),
                              sample_rate=sample_rate, t0=0)
        ...
        for i, p in enumerate(injection_parameters):
            ...
            ra = p['ra']
            dec = p['dec']
            # start of the segment to be injected into
            start_time = (i * args.segment_length) * sample_rate
            geocent_time = t0 + start_time
            p['geocent_time'] = 0 * sample_rate * args.segment_length / \
                2  # center the injection in the middle of the signal
            psi = p['psi']
            ...
            for mode, polarization in polarizations.items():
                ...
                # approximate tricks for finding center and doing injection
                # based on that
                if center_type == None:  # just none for the sine gaussian models
                    # this works for sine gaussian but can be problematic for
                    # other ones
                    centre = np.argmax(np.abs(polarization))
                    # want center to be halfway through the segment, at least
                    # for now
                    a = start_time + N_datapoints // 2 - centre
                    # truncate so it doesn't go over the edge, should be ok
                    # since all zeros
                    inj_slice = slice(a, a + centre * 2)
                    injection[inj_slice] += response * \
                        polarization[:centre * 2]
                elif center_type in ['BBH', 'bbh']:
                    # better idea, put the end of the BBH at the middle? that
                    # way clipping isn'ty an issue
                    injection[start_time:start_time + args.segment_length * sample_rate //
                              2] += response * polarization[-(args.segment_length * sample_rate // 2):]
...
def whiten_bkgs(bkg_segs_full: np.ndarray, sample_rate: int, H1_asd, L1_asd):
    ...
    for i, ifo in enumerate(["H1", "L1"]):
        ...
        for i, bkg_seg in enumerate(bkg_segs):
            white_seg = TimeSeries(
                bkg_seg, sample_rate=sample_rate).whiten(asd=ASDs[ifo])
...
def main(args):
    ...
    if args.signal_type == 'bbh':
        # Injection
        assert loaded_data['H1']['data'].t0.value == loaded_data[
            'L1']['data'].t0.value
        ...
        injected_segs = inject_waveforms(bilby.gw.source.lal_binary_black_hole,
                                         bkg_segs, args.fs, loaded_data['H1'][
                                             'data'].t0.value, args.segment_length,
        ...
        # need both ASDs here
        whitened_segs = whiten_bkgs(injected_segs, args.fs, loaded_data[
                                    'H1']['asd'], loaded_data['L1']['asd'])

        np.save(args.output, whitened_segs)

    elif args.signal_type == 'sg':
        bkg_segs = get_bkg_segs(loaded_data, data, N, args.segment_length)
    ...
    elif args.signal_type == 'background':
        bkg_segs = get_bkg_segs(loaded_data, data, N, args.segment_length)

        whitened_segs = whiten_bkgs(bkg_segs, args.fs, loaded_data['H1'][
                                    'asd'], loaded_data['L1']['asd'])
        np.save(args.output, whitened_segs)

    elif args.signal_type == 'glitch':
        loud_times_H1 = get_loud_segments(
            loaded_data['H1'], N, args.segment_length)
        loud_times_L1 = get_loud_segments(
            loaded_data['H1'], N, args.segment_length)
        ...
        whitened_segs = whiten_bkgs(glitch_segs, args.fs, loaded_data[
                                    'H1']['asd'], loaded_data['L1']['asd'])
