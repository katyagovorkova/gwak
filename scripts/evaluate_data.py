...
def full_evaluation(data, model_folder_path, device, return_midpoints=False):
    ...
    slice_midpoints = np.arange(
        SEG_NUM_TIMESTEPS // 2,
        segments.shape[1] * (SEGMENT_OVERLAP) + SEG_NUM_TIMESTEPS // 2,
        SEGMENT_OVERLAP,
    )
    ...
    quak_predictions = stack_dict_into_tensor(quak_predictions_dict, device=device)
...
def main(args):
    ...
    _, timeaxis_size, feature_size = (
        full_evaluation(data[:2], args.model_paths, DEVICE).cpu().numpy().shape
    )
    ...
    for i in range(n_splits):
        result[DATA_EVAL_MAX_BATCH * i : DATA_EVAL_MAX_BATCH * (i + 1)] = (
            full_evaluation(
                data[DATA_EVAL_MAX_BATCH * i : DATA_EVAL_MAX_BATCH * (i + 1)],
                args.model_paths,
                DEVICE,
            )
            .cpu()
...
if __name__ == "__main__":
    ...
    # Required arguments
    parser.add_argument(
        "data_path", type=str, help="Directory containing the injections to evaluate"
    )

    parser.add_argument(
        "save_path", type=str, help="Folder to which save the evaluated injections"
    )

    parser.add_argument("model_paths", nargs="+", type=str, help="List of models")
