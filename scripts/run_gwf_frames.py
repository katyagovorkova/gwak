...
def whiten_bandpass_resample(file, sample_rate, bandpass_low, bandpass_high, savedir):
    ...
    # split strainL1 and strainH1 into 3 equal-sized parts each for GPU data
    # loading
    ...
    if 0:
        ...
        np.save(savedir + "/H1_BurstBenchmark_%s.npy" %
                int(t0 + (split_size / 4096)), strain_H1_2)
        np.save(savedir + "/H1_BurstBenchmark_%s.npy" %
                int(t0 + (2 * split_size / 4096)), strain_H1_3)
        np.save(savedir + "/L1_BurstBenchmark_%s.npy" % int(t0), strain_L1_1)
        np.save(savedir + "/L1_BurstBenchmark_%s.npy" %
                int(t0 + (split_size / 4096)), strain_L1_2)
        np.save(savedir + "/L1_BurstBenchmark_%s.npy" %
                int(t0 + (2 * split_size / 4096)), strain_L1_3)
    ...
    midpoints = []
    for i in range(3):
        ev, midp = full_evaluation(
            strain[i:i + 1], model_paths, return_midpoints=True)
    ...
    params = np.load(
        "/home/ryan.raikman/s22/forks/katya/gw-anomaly/output/lstm/trained/final_metric_params.npy")
    means, stds = np.load(
        "/home/ryan.raikman/s22/forks/katya/gw-anomaly/output/lstm/trained/norm_factor_params.npy")
    ...
    if len(midpoints[final < final_metric_bar]) == 0:
        return None  # no significant signal found
    ...
    for j in range(len(midpoint_clusters)):
        ...
        quak_evals_ts = np.linspace(
            0, n_points * (SEGMENT_OVERLAP / SAMPLE_RATE), n_points)
        if len(evals[loudest - left_edge:loudest + right_edge, i]) != len(quak_evals_ts):
            continue  # bypassing edge effects

        fig, axs = plt.subplots(1, 2, figsize=(18, 6))
        for i in range(5):
            axs[0].plot(quak_evals_ts * 1000, evals[loudest -
                                                    left_edge:loudest + right_edge, i], label=labels[i])
        axs[0].plot(quak_evals_ts * 1000, final[loudest -
                                                left_edge:loudest + right_edge], label="final metric")
        axs[0].legend()
        ...
        strain_ts = np.linspace(
            0, (right_edge + left_edge) / SAMPLE_RATE, right_edge + left_edge)
        axs[1].plot(strain_ts * 1000, strains[0, p - left_edge:p +
                                              right_edge], label="Hanford", alpha=0.8)
        axs[1].plot(strain_ts * 1000, strains[1, p - left_edge:p +
                                              right_edge], label="Livingston", alpha=0.8)
    ...
    # peak GPS, start GPS, end GPS, characteristic_frequency, lower_frequency,
    # upper_frequency, ranking_statistic_value, false_alarm_rate
    ...
    for j in range(len(midpoint_clusters)):
        ...
        # 2.5 seconds to the left, 5 seconds to the right (since "loudest" is
        # more or less the start time)
        search_window = int(2.5 * SAMPLE_RATE / SEGMENT_OVERLAP)
        search_space = np.arange(
            loc - search_window, loc + 2 * search_window, SEGMENT_OVERLAP)
        ...
        for s in search_space:
            ...
            # s = np.where(midpoints==x)[0][0] #convert to an index for final
            # array

            # also make sure that the considered window has not extended
            # outside the segment
            if s >= 0 and s < len(final) and final[s] < final_metric_bar:
        ...
        print("start, lowest_val, lowest, end",
              start, lowest_val, lowest_midp, end)
...
def main():
    ...
    for file in lst_noduplicates:
        whiten_bandpass_resample(
            file, sample_rate, bandpass_low, bandpass_high, savedir)
