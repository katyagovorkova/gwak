...
def mae_torch_(a, b):
    ...
    norm = np.abs(
        np.sum(
            np.multiply(np.conjugate(np.fft.rfft(a, axis=2)), np.fft.rfft(a, axis=2)),
            axis=2,
        )
    ).mean(axis=1)
    ...
    result = (
        (
            torch.abs(
                torch.sum(
                    torch.multiply(
                        torch.conj(torch.fft.rfft(a, dim=2, n=256)),
                        torch.fft.rfft(b, dim=2, n=256),
                    ),
                    dim=2,
                )
            )
...
def stack_dict_into_tensor(data_dict, device=None):
    ...
    for class_name in data_dict.keys():
        ...
        if RETURN_INDIV_LOSSES:
            stacked_tensor[
                :, stack_index * SCALE : stack_index * SCALE + SCALE
...
def reduce_to_significance(data):
    ...
    for i in range(N_splits):
        ...
        computed_significance[:, start:end] = (
            data[:, start + norm_datapoints : end + norm_datapoints] - means[i]
        ) / stds[i]
...
def load_folder(
    path: str, load_start: int = None, load_stop: int = None, glitches: bool = False
):
    """
    ...
    for ifo in IFOS:
        ...
        for file in os.listdir(triggers_path):
            if file[-3:] == ".h5":
                ...
                if (
                    trigger_start_time > min_trigger_load
                    and trigger_start_time < max_trigger_load
                ):
        ...
        data = TimeSeries(
            X, sample_rate=sample_rate, t0=start if glitches else start + load_start
        )
        ...
        loaded_data[ifo] = {
            ...
            "data": data,
            "asd": data.asd(
                fftlength=fftlength, overlap=0, method="welch", window="hanning"
            ),
        }
...
def slice_bkg_segments(
    ifo_data: dict, data, start_times: list[float], segment_length: float
):
...
def whiten_bandpass_bkgs(
    bkg_segs_full: np.ndarray, sample_rate: int, H1_asd, L1_asd, clip_edge: int = 1
):

    ASDs = {"H1": H1_asd, "L1": L1_asd}
    all_white_segs = []
    for i, ifo in enumerate(IFOS):
        bkg_segs = bkg_segs_full[i]
        final_shape = (
            bkg_segs.shape[0],
            bkg_segs.shape[1] - 2 * int(clip_edge * sample_rate),
        )
        ...
        for j, bkg_seg in enumerate(bkg_segs):
            if BANDPASS_HIGH == SAMPLE_RATE // 2:
                ...
                white_seg = (
                    TimeSeries(bkg_seg, sample_rate=sample_rate)
                    .whiten(asd=ASDs[ifo])
            ...
            white_segs[j] = clipping(white_seg, sample_rate, clip_edge=clip_edge)
...
def get_background_segs(loaded_data, data, n_backgrounds, segment_length):
    ...
    bkg_segs, _ = slice_bkg_segments(
        loaded_data["H1"], data, quiet_times, segment_length
    )
    return bkg_segs
...
def calc_SNR_new(datae, detec_data, fs, detector_psds=None, return_psds=False):
    # heavily inspired by:
...
def inject_hplus_hcross(
    ...
    for i, ifo in enumerate(IFOS):
        ...
        for (
            mode,
            polarization,
        ) in pols.items():  # make sure that this is formatted correctly
            ...
            if inject_at_end:
                center = int(
                    len(injection) - SAMPLE_RATE * EDGE_INJECT_SPACING - half_polar
                )
    ...
    if get_psds:
        _, psds = calc_SNR_new(
            final_bothdetector_nonoise, background, sample_rate, return_psds=True
        )
        return psds
    computed_SNR = calc_SNR_new(
        final_bothdetector_nonoise, background, sample_rate, detector_psds=detector_psds
    )
...
def olib_time_domain_sine_gaussian(
    ...
    **kwargs,
    ...
    hplus, hcross = BurstSineGaussian(q, frequency, hrss, eccentricity, phase, dt)
...
def WNB(duration, fs, fmin, fmax, enveloped=True, sidePad=None):
    ...
    if isinstance(sidePad, bool):
        ...
        raise TypeError(
            "sidePad can be boolean or int value."
    ...
    **kwargs: Any keyword arguments acompany the option of envelope.
    ...
    if option == "sigmoid":
        ...
        env = np.hstack(
            (
                envStart,
                np.ones(len(strain) - len(envStart) - len(envEnd)) - 0.01,
                envEnd,
            )
        )
    ...
    if option == "wnb":
        ...
        wnb = (
            1.2
            + np.random.rand() * 1.8
            + WNB(duration=duration, fs=fs, fmin=fmin, fmax=fmax)
        )
        env = wnb * np.hstack(
            (
                envStart,
...
def clean_gw_events(event_times, data, ts, tend):
...
def split_into_segments(data, overlap=SEGMENT_OVERLAP, seg_len=SEG_NUM_TIMESTEPS):
    """
...
def split_into_segments_torch(
    data, overlap=SEGMENT_OVERLAP, seg_len=SEG_NUM_TIMESTEPS, device=None
):
    """
    Function to slice up data into overlapping segments
    seg_len: length of resulting segments
    ...
    result = torch.empty((n_batches, N_slices, n_detectors, seg_len), device=device)
    ...
    for family_index in range(family_count):
        ...
        result[:, family_index:N_slices_limited:family_count, 0, :] = data[
            :, 0, offset_families[family_index] : end
        ].reshape(n_batches, -1, SEG_NUM_TIMESTEPS)
        result[:, family_index:N_slices_limited:family_count, 1, :] = data[
            :, 1, offset_families[family_index] : end
        ].reshape(n_batches, -1, SEG_NUM_TIMESTEPS)
...
def pearson_computation(
    ...
    for family_index in range(family_count):
        ...
        hanford = data[:, 0, offset_families[family_index] : end].reshape(
            n_batches, -1, SEG_NUM_TIMESTEPS
        )
        hanford = hanford - hanford.mean(dim=2)[:, :, None]
        best_corrs = -1 * torch.ones(
            (hanford.shape[0], hanford.shape[1]), device=device
        )
        for shift_amount in np.arange(-max_shift, max_shift + shift_step, shift_step):

            livingston = data[
                :, 1, offset_families[family_index] + shift_amount : end + shift_amount
            ].reshape(n_batches, -1, SEG_NUM_TIMESTEPS)
            livingston = livingston - livingston.mean(dim=2)[:, :, None]

            corrs = torch.sum(hanford * livingston, axis=2) / torch.sqrt(
                torch.sum(hanford * hanford, axis=2)
                * torch.sum(livingston * livingston, axis=2)
            )
    ...
    for k, center in enumerate(
        np.arange(
            feature_length - max_shift - seg_len // 2 + seg_step,
        ...
        for shift_amount in np.arange(-max_shift, max_shift + shift_step, shift_step):
            ...
            corr = torch.sum(hanford * livingston, axis=1) / torch.sqrt(
                torch.sum(hanford * hanford, axis=1)
                * torch.sum(livingston * livingston, axis=1)
            )
